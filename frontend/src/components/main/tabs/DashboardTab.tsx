  ...
  return (
    <div className="p-4 space-y-5">
      ...
      <div className="card-neigh text-center animate-fade-up" style={{ animationDelay: '100ms' }}>
        <p className="text-xs text-muted-foreground uppercase tracking-wider font-medium mb-2">Savings Vault 🌾</p>
        <div className="relative w-24 h-24 mx-auto mb-3">
          <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
            <circle cx="50" cy="50" r="42" fill="none" stroke="hsl(var(--border))" strokeWidth="8" />
            <circle
              cx="50" cy="50" r="42" fill="none" stroke="hsl(var(--savings))" strokeWidth="8"
      ...
      <div className="card-neigh animate-fade-up" style={{ animationDelay: '200ms' }}>
        <div className="flex justify-between items-center mb-3">
          ...
          <span className="text-sm font-bold tabular-nums">£{impulseSpent} / £{impulseBudget}</span>
        </div>
        <div className="w-full h-3 bg-secondary rounded-full overflow-hidden">
          <div
            ...
            style={{
              width: `${budgetPercent}%`,
              backgroundColor: budgetPercent > 80 ? 'hsl(var(--impulse))' : budgetPercent > 50 ? 'hsl(var(--warning))' : 'hsl(var(--savings))',
            }}
          />
        </div>
        {budgetPercent > 80 && (
        ...
        <div className="flex items-center gap-2 mt-2 text-xs text-muted-foreground">
          ...
          <div className="flex gap-1">
            {[0.1, 0.3, 0.6, 1].map(opacity => (
              <div key={opacity} className="w-3 h-3 rounded-sm" style={{ backgroundColor: `hsl(var(--impulse) / ${opacity})` }} />
            ))}
          </div>
          <span>More</span>
        </div>
      ...
      <div className="card-neigh animate-fade-up" style={{ animationDelay: '400ms' }}>
        ...
        <div className="space-y-3">
          {impulseTransactions.slice(0, 5).map((tx, i) => (
            <div key={tx.id} className="flex items-start justify-between animate-fade-up" style={{ animationDelay: `${450 + i * 60}ms` }}>
              <div>
                <p className="text-sm font-medium">{tx.description}</p>
                <p className="text-xs text-muted-foreground italic">{tx.horseMessage}</p>
              </div>
              <span className="text-sm font-bold text-impulse tabular-nums whitespace-nowrap ml-3">-£{tx.amount.toFixed(2)}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="card-neigh animate-fade-up" style={{ animationDelay: '500ms' }}>
        ...
        <div className="space-y-3">
          {punishments.map((p, i) => (
            <div key={p.id} className="rounded-xl bg-secondary p-3 animate-fade-up" style={{ animationDelay: `${550 + i * 80}ms` }}>
